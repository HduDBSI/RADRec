...
class RADRecTrainer(Trainer):
    ...
    def iteration(self, epoch, dataloader, train=True):
        if train:
            ...
            for _, rec_batch in rec_t_data_iter:
                rec_batch = tuple(t.to(self.device) for t in rec_batch)
                _, input_seq, input_seq_len, target_pos, target, _, entropy_group = rec_batch
                
                x0_gt = self.model.calculate_x0(target)
                x0_out = self.seq_model(input_seq)
                seq_repr = x0_out[:, -1, :]
                ...
                diff_loss, _ = self.calculate_diff_loss(self.model, x0_gt, s, t)
                ...
                if self.args.diff_weight == 0:
                    ...
                    x0_hat_out = self.model.sample_from_reverse_process(s)
                ...
                logits = self.predict_full(seq_repr)
            ...
            for i, batch in rec_data_iter:
                ...
                user_ids, input_ids, _, _, answers, _, _ = batch
